...
class AttributeDeterminedElement(Element):
    def concrete_type_by_attribute(cls, context):
        raise Exception()
    concrete_type_by_attribute = classmethod(concrete_type_by_attribute)


def determine_concrete_model_by_attribute(ctx, record, key_model):
    stream = record.bytestream()
    gen = key_model.concrete_type_by_attribute(ctx)
    type, identifier = gen.next()
    value = type.parse( stream )
    model_type = gen.send(value)
    instance = model_type(ctx, {identifier: value})

    gen = instance.attributes()
    for type, identifier in gen:
        value = type.parse(stream)
...
class BinData(AttributeDeterminedElement):
    def concrete_type_by_attribute(cls, context):
        ...
        yield [BinLink, BinEmbedded, BinStorage][flags & 3]
    concrete_type_by_attribute = classmethod(concrete_type_by_attribute)
...
class CommonControl(Control):
    ...
    def attributes(self):
        ...
        yield ARRAY(context.HWPUNIT16, 4), 'margin',
        yield UINT32, 'instanceId',
        if self.context.version > (5, 0, 0, 4):
...
class Table(CommonControl):
    ...
    def __getattr__(tbl, name):
        if name == 'borderFill':
            return context.mappings[BorderFill][tbl.body.borderFillId - 1] # TODO: is this right?
...
class DocumentContext:
    dpi = 96
    inch_scale = 1
    inch2mm = staticmethod(lambda x: float(int(x * 25.4 * 100 + 0.5)) / 100)
    inch2px = lambda self, x: int(x * self.dpi + 0.5)
    hwp2inch = lambda self, x: x * self.inch_scale / 7200.0
    hwp2mm = lambda self, x: self.inch2mm(self.hwp2inch(x))
    hwp2px = lambda self, x: self.inch2px(self.hwp2inch(x))
    hwp2pt = staticmethod(lambda x: int( (x/100.0)*10 + 0.5)/10.0)

    def _units(context):
        class UNIT_BASE(object):
            def __getattr__(self, name):
                if name == 'inch': return context.hwp2inch(self)
                if name == 'mm': return context.hwp2mm(self)
                if name == 'px': return context.hwp2px(self)
                if name == 'pt': return context.hwp2pt(self)
                raise AttributeError(name)
            def __repr__(self):
                return '%.1fpt(%.1fmm, 0x%x)'%(context.hwp2pt(self), context.hwp2mm(self), self)
        class HWPUNIT(UNIT_BASE, UINT32): pass
        class SHWPUNIT(UNIT_BASE, INT32): pass
        class HWPUNIT16(UNIT_BASE, INT16): pass
        return HWPUNIT, SHWPUNIT, HWPUNIT16
    _units = cached_property(_units)
    _HWPUNIT = lambda self: self._units[0]
    _SHWPUNIT = lambda self: self._units[1]
    _HWPUNIT16 = lambda self: self._units[2]
    HWPUNIT = property(_HWPUNIT)
    SHWPUNIT = property(_SHWPUNIT)
    HWPUNIT16 = property(_HWPUNIT16)
...
#def defineModels(context):
#    class Panose1(Element):
#        def getFields(self):
...
#            yield BYTE, 'xheight',
#
#    class DocumentProperties(Element):
#        def getFields(self):
...
#
#    class FillColorPattern(Element):
#        def getFields(self):
...
#            return '%s\n'%self.__class__.__name__+'\n'.join([ ' - %s = %s'%(name, repr(getattr(self, name))) for type, name in self.getFields()])
#
#    class AlternateFont(Element):
#        def getFields(self):
#            yield BYTE, 'kind'
#            yield BSTR, 'name'
#
#    class FaceName(Element):
...
#                return context.open_bindata(self.name)
#            raise AttributeError(name)
#
#    class IdMappings(Element, dict):
#        def getFields(self):
...
#
#    class Numbering(Element):
#        pass
#
#    class Bullet(Element):
#        pass
#
#    class LineSeg(Element):
#        def getFields(self):
#            yield INT32, 'chpos',
...
#
#    class ParaRangeTag(Element):
#        pass
#
#    class ListHeader(Element):
#        Flags = dataio.Flags(UINT32, (
#            (0, 2), 'textdirection',
#            (3, 4), 'linebreak',
#            (5, 6), 'vertAlign',
...
#            return '%s\n'%self.__class__.__name__+'\n'.join([ ' - %s = %s'%(name, repr(getattr(self, name))) for type, name in self.getFields()])
#
#    class PageBorderFill(Element): pass
#    class ForbiddenChar(Element): pass
#
#    class ShapeLine(Element): pass
#    class ShapePolygon(Element): pass
#    class ShapeEllipse(Element): pass
#    class ShapeTextArt(Element): pass
#    class ShapeOLE(Element): pass
#    class Coord:
#        def getFields(self):
#            yield SHWPUNIT, 'x',
#            yield SHWPUNIT, 'y',
#    class ShapeRectangle(Element):
#        def getFields(self):
#            yield BYTE, 'round',
#            yield ARRAY(Coord, 4), 'coords',
#
#    class PictureInfo(Element):
#        def getFields(self):
...
#        def __getattr__(self, name):
#            if name == 'binData':
#                return context.mappings[BinData][self.binId - 1]
#            raise AttributeError(name)
#
#    class ShapePicture(Element):
#        def getFields(self):
